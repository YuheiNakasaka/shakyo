...
var async = flag.Bool("A", false, "Asynchronizes find")
...
func fileSync(base string) chan string {
	...
	go func() {
		...
		// filepath.Walkは第一引数に指定されたパス配下のファイルを再帰的に情報を取得する。シンボリックリンクは対象にならない。
...
func fileAsync(base string) chan string {
